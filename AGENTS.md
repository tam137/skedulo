...
>    * **Test-Pflicht für neue Features:** Wenn neue Funktionalitäten oder Änderungen an bestehender Logik vorgenommen werden, **muss zwingend** ein entsprechender E2E-Test in `tests/e2e/` hinzugefügt oder erweitert werden. Eine reine Code-Änderung ohne Test-Erweiterung ist unzulässig.
>    * **Mobil- & Responsive-Kompatibilität:** Bei jeder Design- oder Layout-Änderung muss zwingend sichergestellt werden, dass die mobile Darstellung (Responsive Design) nicht beeinträchtigt wird. Neue oder modifizierte Layouts dürfen auf mobilen Viewports keine horizontalen Scrollbalken oder fehlerhafte Element-Breiten erzeugen. Die Verifizierung muss sowohl für Desktop als auch für mobile Ansichten erfolgen.
> 2b. **Führe E2E-Tests lokal aus (Verifizierung):** Starte `./tests/run-tests.sh` und stelle sicher, dass alle Tests erfolgreich durchlaufen.
>    * **Achtung (Fail-Safe-Verhalten):** Solange die E2E-Tests fehlschlagen, **darf der Agent unter keinen Umständen** mit dem Workflow fortfahren, das Changelog aktualisieren oder einen Commit erstellen. Der Agent muss die Fehler im Code oder im Schema analysieren, Korrekturen vornehmen und die Tests so lange wiederholen, bis sie alle erfolgreich (grün) durchlaufen.
...
> 6. **Kein automatisches Live-Deployment:** Führe nach dem Commit und Push kein Deployment auf den Live-Server aus. Das Auschecken und Bereitstellen der Änderungen auf dem Server wird vom Benutzer manuell durchgeführt.
>
...
4. **Responsive Design & CSS-Kaskade (`!important` Verbot):** Bei allen CSS-Anpassungen muss die mobile Version und deren Layout immer zwingend mitberücksichtigt werden. Es gilt ein **absolutes Verbot** der Nutzung von `!important` im CSS, ohne jegliche Ausnahmen. Konflikte und Überschreibungen MÜSSEN zwingend über die korrekte CSS-Spezifität und die Lade-Reihenfolge (Kaskade) gelöst werden. Jede Komponenten-Datei (z. B. `tables.css`, `modals.css`) behält ihre eigenen Media-Queries (z. B. für Mobile-Ansichten). Diese Media-Queries müssen zwingend **ganz am Ende** der jeweiligen Datei platziert werden, damit die Kaskade greift und Basis-Styles sauber überschrieben werden.
